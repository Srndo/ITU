...
(function($) {
  $.Peelback = function(el, settings) {
    ...
    base.init = function() {
      ...
      bigMaskSize = Math.floor((base.settings.bigSize - 10)) + 'px';
      
      //Assemble
      peelHTML = $('<div id="peelback"><a href="' + base.settings.clickURL + '" target="_blank" onclick="_gaq.push([\'_trackEvent\', \'Ad_Interaction\', \'Peelback-click\', \'' + base.settings.gaLabel+ '\']);"><img src="' + base.settings.peelImage +'" alt="" border="0" /></a><div></div></div>');
...
$(document).ready(function(){
    ...
    $("#fanclub_messages_wrapper .del_fanclub_message").click(function(event) {
        if (confirm('Opravdu chceš smazat tento komentář?')) {
            $.get($(this).attr('href'));
    ...
    $("#fanclub_messages_wrapper .add_comment").live('click', function(event) {
        if ($('#new_comment').length == 0) {
            ...
            var html = '<div data-parent="' + id + '" class="fcmsg comment" id="new_comment"> \
                            ...
                            <textarea class="autoresize" name="text" id="comment" placeholder="Napiš komentář..."></textarea> \
    ...
    $('#new_comment form').live('submit', function() {        
        ...
        if (msg.length == 0) {
            alert('Musíš zadat nějaký komentář');
            return false;
        } else {
...
$(document).ready(function() {
    ...
    $('input#input').autocomplete({
        ...
        select: function(event, ui) {
            ...
            var protocol = 'https://';
            if ((hostname === 'karaoke.localhost') || (hostname === 'karaoke.localhost333')) {
                protocol = 'http://';
            }
...
function onEmbedPlayerStateChange(event) {   
    ...
    if (ytplayerType == 'show-lyrics') {
        ...
        if (stateG == 1) {
            ...
            $("#start_again").attr('value', 'Začít úplně znovu');
        } else {
...
$(document).ready(function() 
{
    ...
    $("ul#karaoke_stars_rating a").click(function(event){
        ...
        if (signedG == 'false') {
            ...
            $.get(url, function(data) {                
                $('p.karaoke_rating').html('Průměrné hodnocení: <strong>' + data + '</strong>');
    ...
    $("#karaoke_editor img.add").live('click', function(event){
        ...
        if (ytplayerType == 'edit-karaoke') str += "<img title='Vloží k řádku aktuální čas' class='now' alt='Teď' src='/img/cs_CZ/now.jpg'>";
        str +="<input class='time' type='text' name='time[]' value='' />\n\
<input class='text' type='text' name='text[]' value='' />\n\
<img class='del' title='Odebrat řádek' alt='Odebrat řádek' src='/img/del_row.jpg'>\n\
<img class='add' title='Přidat řádek' alt='Přidat řádek' src='/img/add_row.jpg'><div class='clear'></div></div>"
    ...
    $("#start_again").click(function(event) {
        ...
        $("#next_row").removeAttr('disabled');
        $("#next_row").focus();
        $(this).attr('value', 'Začít úplně znovu');
        createIndexG = 0;
...
function checkSylt()
{
    ...
    if (!result) alert("Časy musí jít po sobě a všechny musí být vyplněné.");
    ...
    $('#add_next_artist a').live('click', function(event) {
        $('#artists_autocomplete').append('<div><dt><label for="artists" class="optional">&nbsp;</label></dt>\n\
            <dd><input type="text" name="artists[]" value="" style="float: left;" />\n\
               <a href="#"><img class="remove_artist" alt="Přidat účinkujícího" src="/img/remove_small.png"></a>\n\
            </dd></div>');
...
$(document).ready(function() {
    ...
    $('#manage_videos a.set_default').live('click', function(event) {
        ...
        $.get($(this).attr('href'), function(data) {
            $('#manage_videos p.default_video').each(function() {
...
$(document).ready(function() {
    ...
    $(document).on('click', '#videos_box a', function(event) {
        ...
        //$(document).on('click', '.alternative_video', function(event) {   // pak musíš změnit "this" i v else
        ...
        // change iframe URL
        if ((hostname === 'karaoke.localhost') || (hostname === 'www.karaoketexty.cz') || (hostname === 'www.karaoketexty.sk') || (hostname === 'www.karaoke-lyrics.net') || (hostname === 'www.karaoke-texte.de') || (hostname === 'www.karaoketeksty.pl')) {
...
//$(document).ready(function() {
    ...
    // !!! DEL MESSAGES z messages.js NA LOCALE NEFUNGUJE OVĚŘ NA OSTRÉM (to zde zakomentuj)
...
//$(document).ready(function() {
	...
	$('.comment_send_button').on('click', function(event)
	{
		...
		// když blbne ALERT odstraň   , 'json' -- jde o chybu ve vracenych datech
